...
importMiniui(function () {
    ...
    require(["miniui-tools"], function (tools) {
        ...
        tools.initGrid(grid);
        grid.setUrl(API_BASE_PATH + "match");
        function search() {
        ...
        $(".add-button").click(function () {
            tools.openWindow("admin/match/save.html", "编辑牌局", "650", "430", function (e) {
                grid.reload();
            })
        });
        search();
...
function edit(id) {
    tools.openWindow("admin/match/save.html?id=" + id, "编辑牌局", "650", "430", function (e) {
        grid.reload();
    })
}
        ...
        tools.createActionButton("删除", "icon-remove", function () {
            if (row._state == "added") {
                ...
                require(["request", "message"], function (request, message) {
                    message.confirm("确定删除该牌局?", function () {
                        var loading = message.loading("删除中...");
                        request["delete"]("match/" + row.id, {}, function (res) {
                            loading.close();
                            if (res.status == 200) {
                                e.sender.removeNode(row);
                            } else {
